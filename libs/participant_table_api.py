...
from constants.common_constants import API_DATE_FORMAT, DEV_TIME_FORMAT
from constants.data_access_api_constants import EXTRA_TABLE_FIELDS, TABLE_QUERY_FIELDS
...
def zip_extra_fields_into_participant_table_data(
    ...
    """ Grabs the extra fields for the participants and adds them to the table. """
    ...
    extra_fields_datetimes: List[Tuple[Optional[datetime]]] = list(
    ...
    participants_datetimes: Tuple[Optional[datetime]]
    for i, participants_datetimes in enumerate(extra_fields_datetimes):
        # this is too hard to debug or read if we try to compact this into a list comprehension
        extra_fields_strings: List[str] = []
        for optional_datetime in participants_datetimes:
            if optional_datetime:
                formatted_dt = optional_datetime.strftime(DEV_TIME_FORMAT)
            else:
                formatted_dt = "None"
            extra_fields_strings.append(formatted_dt)

        # the table data is exactly the length and order of the participant extra fields.
        table_data[i].extend(extra_fields_strings)
